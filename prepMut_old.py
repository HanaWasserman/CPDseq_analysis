...
praser.add_argument('-f', '--path_to_mutation_data')
...
def main(old_tf, tf_txt, lng, sz):
    ...
    tf_d_c[(half-shft-sz):(half+shft+sz)].to_csv(f'mutations/{tf_txt}_skcm_mutation_counts_{sz}.csv', index=False, sep='\t') #OUTPUT1


motifs = pd.read_csv(f'TFs/{args.transcription_factors}', usecols=['Width', 'Name_str'])
motifs_r = motifs.to_records(index=False)
#fold_txt = '/usr/project/xtmp/hs239/mutation_counts/skcm'
